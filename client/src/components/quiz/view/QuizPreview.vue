<template>
  <q-card class="no-padding no-margin q-pa-md items-start">
    <q-card-section>
      ...
      <div class="text-h6 text-orange-9">Title: {{ quiz.title | trimto(30) }}</div>
      <div class="no-margin text-h7 q-mt-sm q-mb-xs">{{ quiz.description | trimto(55) }}</div>
      <q-item dense class="no-padding no-margin ">
        <q-item-label class="no-padding no-margin">Tags:&nbsp;[&nbsp;</q-item-label>
        <q-item-label class="no-padding no-margin" v-for="(tag, index) in quiz.tagList" :key="tag">
          <span>{{tag}}</span>
          <span v-if="index+1 < quiz.tagList.length">&nbsp;|&nbsp;</span>
          <span v-else>&nbsp;]</span>
        </q-item-label>
      </q-item>
      ...
      <div class="text-grey-6">
        <div class="row">
          ...
          <div class="no-padding col-md-4 offset-md-4">
            Audiance: {{ quiz.audiance | trimto(20) }}
            <br>
            Subject: {{ quiz.subject | trimto(20) }}
            <br>
