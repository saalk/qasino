...
public enum GameState implements LabeledEnum {
    ...
    // new
    NEW("new", "New games may not have PlayingQuizs, Leagues, Players","NEW"),
    ...
    public static Set<GameState> quizGamesNew = EnumSet.of(NEW, PENDING_INVITATIONS, PREPARED);
    public static String[] quizGamesNewValues = new String[]{NEW.name(),
            PENDING_INVITATIONS.name(), PREPARED.name()};

    public static Set<GameState> quizGamesStarted = EnumSet.of(PLAYING);
    public static String[] quizGamesStartedValues = new String[]{PLAYING.name()};

    public static Set<GameState> quizGamesFinished = EnumSet.of(FINISHED, QUIT, CANCELLED, OLD);
    public static String[] quizGamesFinishedValues = new String[]{FINISHED.name(), QUIT.name(),
            CANCELLED.name(), OLD.name()};

    public static Set<GameState> quizGamesError = EnumSet.of(TIMEOUT, ERROR);
    public static String[] quizGamesErrorValues = new String[]{TIMEOUT.name(), ERROR.name(),
            CANCELLED.name()};
