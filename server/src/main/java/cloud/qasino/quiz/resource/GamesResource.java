...
import static cloud.qasino.quiz.configuration.Constants.DEFAULT_PAWN_SHIP_BOT;
import static cloud.qasino.quiz.configuration.Constants.isNullOrEmpty;
...
public class GamesResource {
    ...
    public ResponseEntity<Game> setupInitGameInLEagueWithUserAndPlayer(
        ...
        List<Quiz> quizzes =
                quizRepository.saveAll(startedGame.getQuizs());
        if (startedGame.getGameId() == 0 || isNullOrEmpty(quizzes)) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).headers(headers).build();
        }
