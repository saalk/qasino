...
create table "cardmove" ("bet" integer, "end_fiches" integer, "start_fiches" integer, "card_id" bigint, "cardmove_id" bigint generated by default as identity, "gamingtable_id" bigint not null, "player_id" bigint, "created" varchar(25), "card_move_details" varchar(255), "location" varchar(255) check ("location" in ('PACK','STOCK','STACK','HAND','PILE_1','PILE_2','TABLEAU_1','WASTEPILE','ERROR')), "move" varchar(255) not null check ("move" in ('DEAL','PASS','NEXT','STOP','PLAY','STAND','DOUBLE','HIGHER','LOWER','ERROR')), "sequence" varchar(255), primary key ("cardmove_id"));
...
create table "gamingtable" ("current_move_number" integer, "current_round_number" integer, "current_seat_number" integer, "month" tinyint check ("month" between 0 and 11), "week" varchar(3), "weekday" integer, "year" integer, "game_id" bigint not null unique, "gamingtable_id" bigint generated by default as identity, "player_id" bigint not null unique, "created" varchar(25), primary key ("gamingtable_id"));
...
create index "gamingtable_game_index" on "gamingtable" ("game_id");
...
alter table if exists "cardmove" add constraint fk_gamingtable_id foreign key ("gamingtable_id") references "gamingtable";
alter table if exists "game" add constraint fk_league_id foreign key ("league_id") references "league";
alter table if exists "gamingtable" add constraint fk_player_id foreign key ("player_id") references "player";
alter table if exists "gamingtable" add constraint fk_game_id foreign key ("game_id") references "game";
