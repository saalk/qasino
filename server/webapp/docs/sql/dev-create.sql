...
create table "turn" ("current_move_number" integer, "current_round_number" integer, "current_seat_number" integer, "month" tinyint check ("month" between 0 and 11), "week" varchar(3), "weekday" integer, "year" integer, "game_id" bigint not null unique, "player_id" bigint not null unique, "turn_id" bigint generated by default as identity, "created" varchar(25), primary key ("turn_id"));
...
alter table if exists "turn" add constraint fk_player_id foreign key ("player_id") references "player";
