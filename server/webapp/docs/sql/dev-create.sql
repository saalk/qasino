...
create table "player" ("fiches" integer, "is_human" boolean, "is_winner" boolean, "seat" integer, "game_id" bigint, "player_id" bigint generated by default as identity, "visitor_id" bigint, "role" varchar(20) not null check ("role" in ('INITIATOR','INVITED','ACCEPTED','REJECTED','BOT','ERROR')), "created" varchar(25), "ai_level" varchar(50) check ("ai_level" in ('DUMB','AVERAGE','SMART','HUMAN','ERROR')), "avatar" varchar(50) check ("avatar" in ('ELF','MAGICIAN','GOBLIN','ROMAN','ERROR')), "avatar_name" varchar(50), primary key ("player_id"));
...
create table "turn" ("current_round_number" integer, "current_turn_number" integer, "month" tinyint check ("month" between 0 and 11), "week" varchar(3), "weekday" integer, "year" integer, "active_player_id" bigint, "game_id" bigint not null unique, "turn_id" bigint generated by default as identity, "created" varchar(25), primary key ("turn_id"));
create table "users_roles" ("role_id" bigint not null, "visitor_id" bigint not null);
create table "visitor" ("alias_seq" integer, "balance" integer, "enabled" boolean not null, "is_using2fa" boolean not null, "month" tinyint check ("month" between 0 and 11), "secured_loan" integer, "week" varchar(3), "weekday" integer, "year" integer, "visitor_id" bigint generated by default as identity, "created" varchar(25), "username" varchar(25) not null unique, "alias" varchar(50) not null, "email" varchar(50) not null, "password" varchar(60), primary key ("visitor_id"));
...
alter table if exists "turn" add constraint fk_game_id foreign key ("game_id") references "game";
