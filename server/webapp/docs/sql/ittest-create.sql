...
create table if not exists "gamingtable" (
"gamingtable_id" bigint generated by default as identity,
"created" varchar(25),
"player_id" bigint,
...
create index if not exists "gamingtable_game_index" on "gamingtable" ("game_id");
...
alter table if exists "cardmove" add constraint if not exists fk_gamingtable_id foreign key ("gamingtable_id") references "gamingtable";
...
alter table if exists "gamingtable" add constraint if not exists fk_player_id foreign key ("player_id") references "player";
alter table if exists "gamingtable" add constraint if not exists fk_game_id foreign key ("game_id") references "game";
