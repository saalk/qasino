-- for mariadb remove the " + card_id bigint auto_increment primary key,
-- or "visitor_id" bigint generated by default as identity,
-- for others do "+ "role_id" bigint not null, primary key ("role_id")

create sequence "privilege_seq" start with 1 increment by 50;
create sequence "role_seq" start with 1 increment by 50;
create table "card" ("rank_suit" varchar(3) not null, "sequence" integer, "card_id" bigserial not null, "game_id" bigint not null, "player_id" bigint unique, "created" varchar(25), "face" varchar(255) not null check ("face" in ('UP','DOWN','ERROR')), "location" varchar(255) not null check ("location" in ('PACK','STOCK','STACK','HAND','PILE_1','PILE_2','TABLEAU_1','WASTEPILE','ERROR')), "position" varchar(255) not null check ("position" in ('SHUFFLED','ORDERED','MANUAL','ERROR')), primary key ("card_id"));
create table "cardmove" ("bet" integer, "end_fiches" integer, "start_fiches" integer, "card_id" bigint, "cardmove_id" bigserial not null, "player_id" bigint, "playing_id" bigint not null, "created" varchar(25), "card_move_details" varchar(255), "location" varchar(255) check ("location" in ('PACK','STOCK','STACK','HAND','PILE_1','PILE_2','TABLEAU_1','WASTEPILE','ERROR')), "move" varchar(255) not null check ("move" in ('DEAL','PASS','NEXT','STOP','PLAY','STAND','DOUBLE','HIGHER','LOWER','ERROR')), "sequence" varchar(255), primary key ("cardmove_id"));
create table "game" ("ante" integer, "month" smallint check ("month" between 0 and 11), "week" varchar(3), "weekday" integer, "year" integer, "game_id" bigserial not null, "initiator" bigint, "league_id" bigint, "style" varchar(10), "updated" varchar(25) not null, "previous_state" varchar(50) check ("previous_state" in ('INITIALIZED','PENDING_INVITATIONS','PREPARED','STARTED','INITIATOR_MOVE','INVITEE_MOVE','BOT_MOVE','FINISHED','STOPPED','CANCELLED','ERROR')), "state" varchar(50) not null check ("state" in ('INITIALIZED','PENDING_INVITATIONS','PREPARED','STARTED','INITIATOR_MOVE','INVITEE_MOVE','BOT_MOVE','FINISHED','STOPPED','CANCELLED','ERROR')), "type" varchar(50) not null check ("type" in ('HIGHLOW','BLACKJACK','ERROR')), primary key ("game_id"));
create table "league" ("is_active" boolean, "name_seq" integer, "league_id" bigserial not null, "visitor_id" bigint not null, "created" varchar(25), "ended" varchar(25), "name" varchar(50) not null, primary key ("league_id"));
create table "player" ("fiches" integer, "is_human" boolean, "is_winner" boolean, "seat" integer, "start_fiches" integer, "game_id" bigint, "player_id" bigserial not null, "visitor_id" bigint, "role" varchar(20) not null check ("role" in ('INITIATOR','INVITED','INVITEE','REJECTED','BOT','ERROR')), "created" varchar(25), "ai_level" varchar(50) check ("ai_level" in ('DUMB','AVERAGE','SMART','HUMAN','ERROR')), "avatar" varchar(50) check ("avatar" in ('ELF','MAGICIAN','GOBLIN','ROMAN','ERROR')), "avatar_name" varchar(50), primary key ("player_id"));
create table "playing" ("current_move_number" integer, "current_round_number" integer, "current_seat_number" integer, "month" smallint check ("month" between 0 and 11), "week" varchar(3), "weekday" integer, "year" integer, "game_id" bigint not null unique, "player_id" bigint not null unique, "playing_id" bigserial not null, "created" varchar(25), primary key ("playing_id"));
create table "privilege" ("privilege_id" bigint not null, "name" varchar(25), primary key ("privilege_id"));
create table "result" ("fiches_won" integer, "month" smallint check ("month" between 0 and 11), "week" varchar(3), "weekday" integer, "winner" boolean, "year" integer, "game_id" bigint not null, "player_id" bigint not null unique, "result_id" bigserial not null, "visitor_id" bigint, "created" varchar(25), "type" varchar(50) not null check ("type" in ('HIGHLOW','BLACKJACK','ERROR')), primary key ("result_id"));
...
create table "visitor" ("alias_seq" integer, "balance" integer, "enabled" boolean not null, "is_using2fa" boolean not null, "month" smallint check ("month" between 0 and 11), "secured_loan" integer, "week" varchar(3), "weekday" integer, "year" integer, "visitor_id" bigserial not null, "created" varchar(25), "username" varchar(25) unique, "alias" varchar(50) not null, "email" varchar(50), "password" varchar(60), primary key ("visitor_id"));
